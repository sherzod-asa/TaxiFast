...
@app.route('/')
def index():
    """Main page with calculator and car categories"""
    min_date = datetime.today().strftime('%Y-%m-%d')
    return render_template('index.html', pricing_tiers=PRICING_TIERS, min_date=min_date)
...
def not_found(error):
    min_date = datetime.today().strftime('%Y-%m-%d')
    return render_template('index.html', pricing_tiers=PRICING_TIERS, min_date=min_date), 404

@app.errorhandler(500)
def server_error(error):
    min_date = datetime.today().strftime('%Y-%m-%d')
    return render_template('index.html', pricing_tiers=PRICING_TIERS, min_date=min_date), 500
